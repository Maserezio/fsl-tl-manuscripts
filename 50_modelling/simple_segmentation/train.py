...
def main(cfg_path: str, overrides: dict | None = None):
    ...
    train_loader = DataLoader(
        ...
        pin_memory=True,
        drop_last=len(train_paired) >= tcfg["batch_size"],
    )
    ...
    run_name = f"simple_segmentation_{dataset_family}_{manuscript}_{os.path.splitext(os.path.basename(cfg_path))[0]}_k{dcfg['k_shot']}"
