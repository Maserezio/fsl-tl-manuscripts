...
from data.diva_dataset import _IMAGENET_MEAN, _IMAGENET_STD, _load_mask, _resolve_image_path, _resolve_layout
...
def _resolve_output_dir(
    ...
    run_name = (
        f"simple_segmentation_{dataset_family}_{manuscript}_"
        f"{Path(cfg_path).stem}_{Path(checkpoint).stem}_{split_key}"
    )
    return repo_root / "99_evaluation" / "simple_segmentation" / run_name


def _binary_metrics(pred_mask: np.ndarray, gt_mask: np.ndarray) -> dict:
...
def _mean_metrics(rows: list[dict], keys: list[str]) -> dict:
    if not rows:
...
def _save_overlay(img_rgb: np.ndarray, pred_mask: np.ndarray, gt_mask: np.ndarray, output_path: Path) -> None:
    overlay = img_rgb.copy().astype(np.float32)
    gt_bool = gt_mask.astype(bool)
    pred_bool = pred_mask.astype(bool)
    overlap = gt_bool & pred_bool

    overlay[gt_bool] = 0.6 * overlay[gt_bool] + 0.4 * np.array([255, 0, 0], dtype=np.float32)
    overlay[pred_bool] = 0.6 * overlay[pred_bool] + 0.4 * np.array([0, 255, 0], dtype=np.float32)
    overlay[overlap] = 0.5 * overlay[overlap] + 0.5 * np.array([255, 255, 0], dtype=np.float32)
    cv2.imwrite(str(output_path), cv2.cvtColor(overlay.astype(np.uint8), cv2.COLOR_RGB2BGR))


def evaluate_u_diads(
    model: UnifiedSegmenter,
    img_dir: str,
    gt_dir: str,
    crop_size: int,
    ...
    prob_map_dir = output_dir / "prob_maps"
    overlay_dir = output_dir / "overlays"
    pred_mask_dir.mkdir(parents=True, exist_ok=True)
    prob_map_dir.mkdir(parents=True, exist_ok=True)
    overlay_dir.mkdir(parents=True, exist_ok=True)

    rows = []
    img_files = sorted(filename for filename in os.listdir(img_dir) if filename.lower().endswith((".jpg", ".jpeg", ".png", ".tif", ".tiff")))

    for filename in tqdm(img_files, desc="Inference [u_diads]"):
        ...
        gt_mask, _ = _load_mask(os.path.join(gt_dir, stem + ".png"), "u_diads")

        cv2.imwrite(str(pred_mask_dir / f"{stem}.png"), pred_mask * 255)
        cv2.imwrite(str(prob_map_dir / f"{stem}.png"), np.clip(prob_map * 255, 0, 255).astype(np.uint8))
        _save_overlay(img_rgb, pred_mask, gt_mask, overlay_dir / f"{stem}.png")

        row = {"image": filename}
        row.update(_binary_metrics(pred_mask, gt_mask))
        rows.append(row)

    metrics_path = output_dir / "mask_metrics.csv"
    _write_metrics_csv(rows, metrics_path)
    summary = _mean_metrics(rows, ["IoU", "Precision", "Recall", "F1", "Dice"])
    summary_path = output_dir / "mask_metrics_summary.csv"
    _write_metrics_csv([summary], summary_path)
    return summary
...
def main(
    ...
    split_key = _canonical_split(split)
    ...
    out_dir = _resolve_output_dir(cfg_path, checkpoint, dataset_family, manuscript, split_key)
    xml_out_dir = out_dir / "pred_xml"
    csv_out_dir = out_dir / "diva_csv"
    ...
    if gt_xml_dir is None:
        raise FileNotFoundError(f"No PAGE XML ground truth directory found for {manuscript} {split_key}")
    ...
    for filename in tqdm(img_files, desc=f"Inference [{split}]"):
        ...
        xml_path = str(xml_out_dir / f"{stem}.xml")
    ...
    if run_eval:
        run_diva_evaluator(
            pred_xml_dir=str(xml_out_dir),
            gt_pixel_dir=gt_pix_dir,
            gt_xml_dir=gt_xml_dir,
            img_dir=img_dir,
            out_csv_dir=str(csv_out_dir),
        )
        summary = summarise_csv_results(str(csv_out_dir))
