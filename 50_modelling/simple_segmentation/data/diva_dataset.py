"""
Datasets for supervised few-shot training with supervoxel maps.
...
--------------------------
1. `diva`
     <data_root>/<manuscript>/
         img-<manuscript>/img/{training,validation,public-test}/*
         pixel-level-gt-<manuscript>/pixel-level-gt/{training,validation,public-test}/*.png
         PAGE-gt-<manuscript>-TASK-2/TASK-2/{training,validation,public-test}/*.xml

2. `u_diads`
     <data_root>/<manuscript>/
         img-<manuscript>/{train|training,val|validation,test}/*
         text-line-gt-<manuscript>/{training,validation,test}/*.png
...
def _load_diva_mask(gt_path: str) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def _load_u_diads_mask(gt_path: str) -> Tuple[np.ndarray, np.ndarray]:
    """
    Load U-DIADS binary GT PNG.

    Returns
    -------
    mask      : uint8 [H, W] — 1 = text line, 0 = background
    dont_care : bool  [H, W] — always False for U-DIADS
    """
    gt = np.array(Image.open(gt_path).convert("L"))
    mask = (gt > 0).astype(np.uint8)
    dont_care = np.zeros_like(mask, dtype=bool)
    return mask, dont_care


def _load_mask(gt_path: str, dataset_family: str) -> Tuple[np.ndarray, np.ndarray]:
    if dataset_family == "diva":
        return _load_diva_mask(gt_path)
    if dataset_family == "u_diads":
        return _load_u_diads_mask(gt_path)
    raise ValueError(f"Unsupported dataset family: {dataset_family}")


def _find_existing_dir(base_dir: str, candidates: List[str]) -> Optional[str]:
...
def _resolve_image_path(img_dir: str, stem: str) -> str:
    for ext in (".jpg", ".jpeg", ".png", ".tif", ".tiff"):
        ...
        if os.path.exists(path):
            return path
    raise FileNotFoundError(f"Cannot find image for stem '{stem}' in {img_dir}")


def _list_image_stems(img_dir: str) -> List[str]:
    stems = []
    for filename in sorted(os.listdir(img_dir)):
        if filename.lower().endswith((".jpg", ".jpeg", ".png", ".tif", ".tiff")):
            stems.append(os.path.splitext(filename)[0])
    return stems


def _resolve_layout(data_root: str, manuscript: str, dataset_family: str) -> Dict[str, Optional[str]]:
    ...
    if dataset_family == "diva":
        ...
        xml_base = _find_existing_dir(
            base,
            [f"PAGE-gt-{manuscript}-TASK-2/TASK-2", f"PAGE-gt-{manuscript}/PAGE-gt"],
        )
        split_candidates = {
            "train": ["training"],
            "val": ["validation"],
    ...
    elif dataset_family == "u_diads":
        img_base = os.path.join(base, f"img-{manuscript}")
        gt_base = os.path.join(base, f"text-line-gt-{manuscript}")
        xml_base = None
        split_candidates = {
    ...
    for split_name, candidates in split_candidates.items():
        img_split_dir = _find_existing_dir(img_base, candidates)
        gt_split_dir = _find_existing_dir(gt_base, candidates)
        xml_split_dir = _find_existing_dir(xml_base, candidates) if xml_base else None
        if img_split_dir is None:
            raise FileNotFoundError(
                f"Could not resolve image directory for split '{split_name}' under {img_base}"
            )
        if gt_split_dir is None:
            raise FileNotFoundError(
                f"Could not resolve GT directory for split '{split_name}' under {gt_base}"
            )
...
class DIVADataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        stem = self.stems[idx]
        img  = _load_image(_resolve_image_path(self.img_dir, stem))
        H, W = img.shape[:2]

        has_gt = self.gt_dir is not None
        if has_gt:
            mask, dc = _load_mask(os.path.join(self.gt_dir, stem + ".png"), self.dataset_family)
        else:
...
def get_splits(
    ...
    def img_dir(split):
        return split_dirs[split]["img"]

    def gt_dir(split):
        return split_dirs[split]["gt"]

    def xml_dir(split):
        return split_dirs[split]["xml"]

    def _all_stems(split):
        return _list_image_stems(img_dir(split))
    ...
    all_train       = _all_stems("train")
    unlabeled_stems = [s for s in all_train if s not in set(labeled_stems)]

    train_l = DIVADataset(
        img_dir("train"), gt_dir("train"),
        labeled_stems, crop_size=crop_size, augment=True,
        xml_dir=xml_dir("train"), dataset_family=dataset_family, **sv_kw,
    )
    train_u = DIVADataset(
        img_dir("train"), gt_dir=None,
        stems=unlabeled_stems if unlabeled_stems else all_train,
        crop_size=crop_size, augment=True, dataset_family=dataset_family, **sv_kw,
    )
    train_paired = SemiSupervisedDIVADataset(train_l, train_u)

    val_ds = DIVADataset(
        img_dir("val"), gt_dir("val"),
        _all_stems("val"), crop_size=None, augment=False,
        xml_dir=xml_dir("val"), dataset_family=dataset_family, **sv_kw,
    )
    test_ds = DIVADataset(
        img_dir("test"), gt_dir("test"),
        _all_stems("test"), crop_size=None, augment=False,
        xml_dir=xml_dir("test"), dataset_family=dataset_family, **sv_kw,
    )
